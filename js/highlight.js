...
(function () {
  ...
  function runMode(code, lang, cb) {
    if (window.CodeMirror && CodeMirror.runMode) CodeMirror.runMode(code, modeOf(lang), cb);
    else cb(code, null);
  }
  ...
  function highlightLines(code, lang = 'cs') {
    ...
    if (window.CodeMirror && CodeMirror.runMode) {
      CodeMirror.runMode(code, modeOf(lang), (text, style) => {
        if (text === '\n') { push(); return; }
    ...
    const MARK = markOf(lang);
    return lines.map((html, i) => {
      const mark = MARK.test(raw[i] || '');
  ...
  window.JU = {
    esc, highlightLines, highlightInline, highlightRepl, fileName, FILE_MARK, FILE_MARK_CPP, markOf, makeEditor, scoped,
    codesOf, blockCodes, expectOf, getLang, setLang, LANG_NAME, modeOf, runMode
  };
})();
