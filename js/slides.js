...
(function () {
  ...
  const LAYOUT_ICON = { title: '🎬', goals: '🎯', bullets: '📌', code: '💻', two: '⚖️', table: '📊', diagram: '🧭', quiz: '❓', practice: '🛠️', summary: '✅', image: '🖼' };
  ...
  class Deck {
    ...
    render() {
      ...
      switch (s.layout) {
        case 'title':
          cls += ' title-slide';
          html = `<div class="badge">${esc(s.badge || `${this.ch.no}차시 · ${this.ch.title}`)}</div>
            <h1>${s.title}</h1>
          ...
          html = top + title + lead + `<div class="s-body"><div class="s-code${s.points && s.points.length ? ' has-points' : ''}">
              <div class="s-editor">
                <div class="s-editor-bar"><span class="fname">📄 ${esc(s.file || fileName(codeText(codes, cur) || '', cur))}</span>${langTabs(codes, cur)}${runnable ? '' : '<span class="s-lang-chip local">🖥 Visual Studio</span>'}
                  ...
                  ${runnable ? '<button class="btn primary small" data-act="run" title="실행 (Ctrl+Enter)">▶ 실행</button>' : ''}</div>
        ...
        case 'image':
          html = top + title + lead + `<div class="s-body"><div class="s-diagram"><div class="d-host"><img class="s-img" src="assets/${esc(s.src)}" alt="" data-act="zoomimg"></div>${s.caption ? `<div class="cap">${s.caption}</div>` : ''}</div></div>` + foot;
          break;
        case 'two': {
          const col = (c, i) => {
        ...
        case 'demo':
        case 'diagram':
          html = top + title + lead + `<div class="s-body"><div class="s-diagram"><div class="d-host">${s.layout === 'demo' ? `<div class="demo-wrap">${s.html}</div>` : JU.scoped(s.html)}</div>${s.caption ? `<div class="cap">${s.caption}</div>` : ''}</div></div>` + foot;
          break;
        case 'quiz':
        ...
        case 'practice': {
          ...
          const codes = slideCodes(s, sol ? 'solution' : 'starter');
          const cur = pick(codes);
          html = top + title + `<div class="s-body"><div class="s-practice"><div class="desc">${s.desc || ''}</div>
              <div class="s-editor">
                <div class="s-editor-bar"><span class="fname">📝 ${sol ? '정답 코드' : '실습 코드'}</span>${langTabs(codes, cur)}
      ...
      if (edHost) {
        ...
        const codes = isPractice ? slideCodes(s, sol ? 'solution' : 'starter') : slideCodes(s);
        const lang = pick(codes);
        const editKey = key + (sol ? ':sol' : '') + ':' + lang;
        const original = codeText(codes, lang) || '';
        const ed = makeEditor(edHost, this.edits[editKey] != null ? this.edits[editKey] : original, { onRun: () => this.runCode(), lang });
      ...
      if (typeof s.init === 'function') {
        try { s.init(slideEl.querySelector('.d-host') || slideEl, window.MLB); } catch (err) { console.error('slide init', err); }
      }
    ...
    onSlideClick(e, s) {
      ...
      if (a === 'zoomimg') { window.MVImage.Viewer.openUrl(act.getAttribute('src'), s.title || ''); return; }
    ...
    onLang() {
      const s = this.slides[this.index];
      if (s && (s.layout === 'code' || s.layout === 'practice')) this.render();
    }
    ...
    runCode(stdin) {
      ...
      this.app.runCode(code, { label: s.title, stdin, editor: this.editor, lang: this.editor.lang });
    }
    ...
    renderNotes() {
      ...
      const solCodes = s.layout === 'practice' ? slideCodes(s, 'solution') : {};
      const solLang = pick(solCodes);
      if (s.layout === 'practice' && codeText(solCodes, solLang) != null && codeText(solCodes, solLang) !== '') {
        answer = `<div class="answer"><b>🔑 정답 코드 (${LANG_TAB[solLang]})</b> <button class="btn small ghost" id="noteRunSol">▶ 정답 실행</button> <button class="btn small ghost" id="noteShowSol">슬라이드에 표시</button>
          <pre style="margin:6px 0 0;font-size:12.5px;overflow:auto;max-height:260px">${highlightInline(codeText(solCodes, solLang), solLang)}</pre></div>`;
      }
      ...
      if (run) run.onclick = () => { this.showConsole(true); this.app.runCode(codeText(solCodes, solLang), { label: s.title + ' (정답)', stdin: s.stdin, lang: solLang }); };
    ...
    broadcast() {
      ...
      this.channel.postMessage({
        type: 'state', lesson: `${this.ch.no}차시 ${this.ch.title} · ${this.sec.title}`, index: this.index, total: this.slides.length,
