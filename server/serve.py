...
class Handler(SimpleHTTPRequestHandler):
    ...
    def end_headers(self):
        ...
        if self.path.startswith('/runtime/'):
            self.send_header('Cache-Control', 'max-age=604800')   # 컴파일러(약 110MB) · OpenCV 라이브러리는 오래 캐시
        elif self.path.split('?')[0].endswith(('.js', '.mjs', '.css', '.html', '.json', '.py')):
            self.send_header('Cache-Control', 'no-store')      # 코드 · 콘텐츠는 항상 새로 (개발 중 수정 즉시 반영)
    ...
    def guess_type(self, path):
        ...
        if path.endswith('.hpp') or path.endswith('.o'):
            return 'application/octet-stream' if path.endswith('.o') else 'text/plain; charset=utf-8'
        if path.endswith('.py'):
