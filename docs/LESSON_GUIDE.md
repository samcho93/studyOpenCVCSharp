...
- C#: 클래스 · 구조체 · 열거형 · 상속 · 인터페이스 · 제네릭 컬렉션(`List<T>` `Dictionary<K,V>` `HashSet<T>` `Queue` `Stack`) · LINQ · 람다 · 예외(try/catch/finally) · 튜플 · `out`/`ref` · 패턴 매칭 · `switch` 식 · 문자열 보간(`$"{x:F2}"`) · `using var` · 지역 함수 · 확장 메서드 호출 · 위임(delegate) 필드와 `event` (`handler += 람다`, `handler?.Invoke(값)`).
